export function computeDiff(beforeImg, afterImg, threshold) {
  ...
  return {
    percentage: ((diffCount / totalPixels) * 100).toFixed(2),
...
export function loadImage(file) {
  return new Promise((resolve) => {
    ...
    const img = new Image();
    img.onload = () => resolve({ url, element: img, name: file.name });
    img.src = url;
  });
}
